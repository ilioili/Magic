...
public class Potatos1View extends GLSurfaceView{
	...
	public Potatos1View(Context context) {
		super(context);
		render=new Potatos1Render();
	...
	public Potatos1View(Context context,AttributeSet paramAttributeSet) {//不加入第二个参数会导致XML解析异常
...
//        			if(Math.abs(dpx)+Math.abs(dpy)>100)	sp.play(music, 1, 1, 0, 0, 1);
	...
	public void fresh(){
		final Potatos1View p=this;
		new Handler().postDelayed(new Runnable(){
			public void run(){
				render.rotateTo(0.1f, 0.2f);
				render.rotateLayerBy(degree++);
				if(degree==45) 	sp.play(music, 1, 1, 0, 0, 1);
