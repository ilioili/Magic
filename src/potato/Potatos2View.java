...
public class Potatos2View extends GLSurfaceView{
	...
	public Potatos2View(Context context) {
		super(context);
		render=new Potatos2Render();
	...
	public Potatos2View(Context context,AttributeSet paramAttributeSet) {//不加入第二个参数会导致XML解析异常
...
//        			if(Math.abs(dpx)+Math.abs(dpy)>100)	sp.play(music, 1, 1, 0, 0, 1);
	...
	public void fresh(){
		final Potatos2View p=this;
		new Handler().postDelayed(new Runnable(){
			public void run(){
				render.rotateTo(0.1f, 0.2f);
				render.rotateLayerBy(degree++);
				if(degree==60&render.potatos.layerIndex>5) 	sp.play(music, 1, 1, 0, 0, 1);
				if(degree==45&render.potatos.layerIndex<6)	sp.play(music, 1, 1, 0, 0, 1);
				if(degree==120&render.potatos.layerIndex>5){
					render.fresh();
					times--;
					Random r=new Random();
					render.setLayer(r.nextInt(13));
					degree=0;
				}else if(degree==90&render.potatos.layerIndex<6){
					render.fresh();
					times--;
					Random r=new Random();
					render.setLayer(r.nextInt(13));
